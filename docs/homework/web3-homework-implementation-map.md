...
| Owner 管理机构和育婴师，Provider 提交并审核上架 | V1 已有 `PROVIDER_ROLE` 与管理员暂停；提交后审核和拒绝状态尚未实现 | `contracts/contracts/TaskMarketplace.sol:21`<br>`web/src/pages/ProviderConsolePage.tsx:1` | `contracts/test/TaskMarketplace.ts:61`；Owner 审核交易缺失 | `partial` |
...
| Chainlink 随机价格和开放时间 | V1 使用 VRF v2.5 两个随机数锁定 2 至 4 BABY 与活动时长 | `contracts/contracts/TaskMarketplace.sol:253`<br>`contracts/contracts/vrf/VRFConsumerBaseV2Plus.sol:1` | `contracts/test/TaskMarketplace.ts:92`；Sepolia request ID 与完成闭环见 `docs/evidence/deployment/2026-08-09-business-closed-loop.json` | `complete` |
| 完成后自动发放带名称和图片的 ERC-5192 证书 | V1 可铸造 ERC-721 且按购买去重；正式 ERC-5192、IPFS 和幂等重复请求尚未实现 | `contracts/contracts/GrowthCertificate.sol:1`<br>`contracts/contracts/TaskMarketplace.sol:225` | V1 Sepolia token ID `1`；`contracts/test/GrowthCertificate.ts`；ERC-5192 接口证据缺失 | `partial` |
