...
| --- | --- | --- | --- |
| `alchemy-app-free-usage.png` | 证明 Alchemy 没有升级或启用付费容量 | 应用为 `babysteps-sepolia`；Active Plan 为 Free；每月额度 30M CU，截图时使用量为 0 | 已裁掉账户团队侧栏；未显示 API Key、RPC URL 或账号邮箱 |
...
| `privy-login-methods-preview.png` | 在 Cloudflare PR 预览站验证真实前端已接入 Privy，而不是仅完成控制台配置 | 个人中心可加载 Privy 弹窗，并提供邮箱、Google、外部钱包三种入口 | 未填写邮箱、未选择账户、未签名；该图证明登录入口可用，不代表已完成最终用户登录 |

对应的机器可读证据：

- `docs/evidence/deployment/2026-08-12-the-graph-sepolia.json`
- `docs/evidence/deployment/2026-08-12-rpc-comparison.json`

补充边界：

- Infura 与 Alchemy 均保持免费方案，且只启用 Ethereum Sepolia；完整 RPC URL/API Key 不进入 Git 或截图。
- RPC 对照脚本已用 PublicNode、Infura、Alchemy 读取同一笔 Sepolia 交易并比对 chain ID、交易、回执与日志；JSON 仅保存脱敏结果和延迟，不保存端点。
- Privy 已验证允许域名和登录方式弹窗；真正的 Google/邮箱认证、嵌入式钱包创建与签名仍需要用户在正式站完成一次交互后，才能标记为端到端完成。
