...
本地实现、单元测试和类型检查已通过。新增 D1 migration、Worker 路由、Web UI 与生产角色尚未发布，因此不得把本记录解读为新的云端或 Sepolia 交易闭环已经完成。

## 要求 → 实现 → 代码 → 证据 → 状态

| 要求 | 实现 | 代码位置 | 本地证据 | 状态 |
| --- | --- | --- | --- | --- |
| Provider 提交、Owner 审核 | Provider 调 V2 `requestTask`；Owner 校验 `DEFAULT_ADMIN_ROLE` 后调用 `approveTask` 或 `rejectTask`，拒绝原因只保存规范化哈希 | `web/src/features/provider/useProviderTaskCreation.ts`、`web/src/features/provider/useOwnerTaskReview.ts`、`web/src/pages/ProviderConsolePage.tsx` | ABI、Provider Hook、Owner Hook 与页面测试通过 | 本地已验证；云端待发布 |
| 购买后才能看视频 | 公共 `GET /api/tasks/:taskKey` 不返回视频；签名 `GET /api/tasks/:taskKey/content` 再核验 `purchaseIdForBuyer` | `worker/src/routes/tasks.ts`、`web/src/features/marketplace/TaskLearningPanel.tsx` | 覆盖未登录、未购买、RPC 失败、已购解锁；RPC 失败按 503 关闭 | 本地已验证；云端待发布 |
| 家长提交任务完成证据 | 登录钱包提交短文本与 HTTPS/IPFS 证书 URI；Worker 重新校验已购事实并计算 canonical keccak hash | `worker/src/routes/completions.ts`、`worker/src/repositories/completionRepository.ts`、`worker/migrations/0003_completion_submissions.sql` | 覆盖未购拒绝、明显 PII 拒绝、相同请求幂等、冲突请求 409 | 本地已验证；D1 migration 待应用 |
...
- 新增 Worker API 与 D1 表必须在 Preview/production 发布后重新做 HTTP、会话、购买门禁和真实钱包确认验收。
