...
- Swap 使用精确额度完成后会重新读取 Router allowance；若仍大于 0，页面锁定新的报价与输入，并要求用户通过独立钱包确认把 allowance 清为 0。链上读回为 0 后才解除清理 Gate。
- Allowance 清理过程中使用同步 pending 锁，重复点击不会发出第二笔交易；交易哈希与最终链上读回分别展示，不把“钱包已确认”冒充“授权已清零”。
- Journey 补偿关系已显式建模：购买成功消费对应 approve；登录/会话的最终登出由 Profile Journey 统一证明；最后通过可见“断开连接”完成钱包 UI 收尾。不可逆的 Sepolia 交易只记录为公开测试历史，不伪称删除。
