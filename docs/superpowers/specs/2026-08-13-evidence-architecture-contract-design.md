...
新增或更新一张 StarBuddy 主题的全局架构图片，覆盖：
...
图例必须区分已实现并验证、已实现待外部验证、计划/延后、降级路径。最终交付必须是可直接展示、放大和保存的图片资产，不得仅以 Mermaid 代码块代替。移动端允许横向滚动或点击打开原图，不得把整张图压缩成不可读缩略图。
...
时序图必须标出失败路径：哈希不一致、未购买、授权不足、VRF pending、交易失败、Relayer 重试和重复完成幂等保护。最终交付同样必须是独立图片资产，Mermaid 仅作为可维护源文件。
